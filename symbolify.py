    ...
    def foo(instr, history):
        ...
        if instr.sourceReg == Register.SP:
            ...
            return InstrResult.write, value, history.lookupAddress(datatype,
                                                                   Expression.build('+', history.read(instr.sourceReg),
...
conversionList = {
    ...
    RegOp.JR: lambda instr, history: (InstrResult.end,) if instr.sourceReg == Register.RA else (
    InstrResult.unhandled, Expression.build('@', 'JR', history.read(instr.sourceReg))),
    ...
    CopOp.BCT: lambda instr, history: (
    InstrResult.branch, Expression.build('!', history.read(SpecialRegister.Compare)), extend(instr.target)),
    CopOp.BCFL: lambda instr, history: (
    InstrResult.likely, history.read(SpecialRegister.Compare), extend(instr.target)),
    CopOp.BCTL: lambda instr, history: (
    InstrResult.likely, Expression.build('!', history.read(SpecialRegister.Compare)), extend(instr.target)),
...
class VariableHistory:
    ...
    def lookupAddress(self, fmt, address):
        ...
        if memOffset >= self.getSize(base.type):
            raise Exception('trying to look up address %#x in %s (only %#x bytes)' % (
            memOffset, base.type, self.getSize(base.type)))
    ...
    def subLookup(self, fmt, prefix, superType, address, others=[]):
        ...
        if others:
            return Symbol(
                '{}({} + {:h})'.format(fmt, prefix, Expression.build('+', Expression('+', others), Literal(address))),
                fmt)
        else:
        ...
        if delayed:
            if delayed[0] in [InstrResult.branch, InstrResult.likely, InstrResult.jump]:
                ...
                if funcCall in bindings['functions']:
                    ...
                    for reg in [Register.A0, FloatRegister.F12, Register.A1, FloatRegister.F14, Register.A2,
                                Register.A3]:
